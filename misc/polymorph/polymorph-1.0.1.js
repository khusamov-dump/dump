	...
	var getPolymorphFunction = function(funcmap) {
		var res = function() {
			...
			for(var i=0; i<flist.length-1; i++) {
				...
				// since flist[i][0] represents the number of arguments in the tested prototype
				// https://code.google.com/p/polymorph-js/issues/detail?id=2
				//for(var j=0; j<flist[i].length; j++) {
				for(var j=0; j<flist[i][0].length; j++) {
